...
private:
    ...
    VWaveformGenerator* dut_ptr = nullptr; // Store DUT pointer for updates

public:
    // Mark methods defined outside the class declaration as inline
    inline CoverageCollector(); // Constructor
    inline void initialize(VWaveformGenerator* top); // Initialization method
...
inline void CoverageCollector::initialize(VWaveformGenerator* top) {
    if (!top) {
        throw std::runtime_error("Error: DUT pointer is null in CoverageCollector::initialize.");
    }
    dut_ptr = top; // Store the pointer
    std::cout << "CoverageCollector: Initializing coverage points for VWaveformGenerator..." << std::endl;
...
// --- BEGIN POPULATING COVERAGE POINTS ---
        try { mux_condition_points.emplace_back("_mux_cond_WaveformGenerator__I__local__I___io_output_T_4", &top->_mux_cond_WaveformGenerator___05FI___05Flocal___05FI___05F_io_output_T_4); } catch(...) { std::cerr << "Error adding mux point _mux_cond_WaveformGenerator___05FI___05Flocal___05FI___05F_io_output_T_4" << std::endl; }
        try { condition_points.emplace_back("_cond_pred_WaveformGenerator__I__local__I___T", &top->_cond_pred_WaveformGenerator___05FI___05Flocal___05FI___05F_T); } catch(...) { std::cerr << "Error adding cond point _cond_pred_WaveformGenerator___05FI___05Flocal___05FI___05F_T" << std::endl; }
        try { condition_points.emplace_back("_cond_pred_WaveformGenerator__I__local__I___T_1", &top->_cond_pred_WaveformGenerator___05FI___05Flocal___05FI___05F_T_1); } catch(...) { std::cerr << "Error adding cond point _cond_pred_WaveformGenerator___05FI___05Flocal___05FI___05F_T_1" << std::endl; }
        try { condition_points.emplace_back("_cond_pred_WaveformGenerator__I__local__I___T_2", &top->_cond_pred_WaveformGenerator___05FI___05Flocal___05FI___05F_T_2); } catch(...) { std::cerr << "Error adding cond point _cond_pred_WaveformGenerator___05FI___05Flocal___05FI___05F_T_2" << std::endl; }
        try { condition_points.emplace_back("_cond_pred_WaveformGenerator__I__local__I___T_3", &top->_cond_pred_WaveformGenerator___05FI___05Flocal___05FI___05F_T_3); } catch(...) { std::cerr << "Error adding cond point _cond_pred_WaveformGenerator___05FI___05Flocal___05FI___05F_T_3" << std::endl; }
        // Register signal: _reg_signals_WaveformGenerator__I__local__I__phaseAcc (UInt<16>)
        register_points.emplace_back("_reg_signals_WaveformGenerator__I__local__I__phaseAcc", 16, reinterpret_cast<const void*>(&top->_reg_signals_WaveformGenerator___05FI___05Flocal___05FI___05FphaseAcc));

// --- END POPULATING COVERAGE POINTS ---

    initialized = true;
    std::cout << "CoverageCollector: Initialization complete." << std::endl;
    std::cout << "  - Conditionally Predicates Registered: " << 4 << " (target: " << condition_points.size() << ")" << std::endl;
    std::cout << "  - Mux Conditions Registered:         " << 1 << " (target: " << mux_condition_points.size() << ")" << std::endl;
    std::cout << "  - Register Coverage Points Registered: " << 1 << " (target: " << register_points.size() << ")" << std::endl; // Note: regPointsCount is after flattening
    std::cout << "----------------------------------------" << std::endl;
}
