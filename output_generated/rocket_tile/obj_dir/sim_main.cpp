...
int main(int argc, char **argv) {
    ...
    try {
        top = std::make_unique<VExampleRocketSystem>(contextp.get(), "DUT");
         ...
         std::cerr << "FATAL ERROR: Failed to instantiate DUT VExampleRocketSystem: " << e.what() << std::endl;
         return 1;
    } catch (...) {
         std::cerr << "FATAL ERROR: Unknown error during DUT VExampleRocketSystem instantiation." << std::endl;
         return 1;
    }
