...
int main(int argc, char **argv) {
  ...
  // Instantiate DUT
  std::unique_ptr<VExampleRocketSystem> top;
  try {
    top = std::make_unique<VExampleRocketSystem>(contextp.get(), "DUT");
  } catch (const std::exception &e) {
    std::cerr << "FATAL ERROR: Failed to instantiate DUT VExampleRocketSystem: "
              << e.what() << std::endl;
    return 1;
  } catch (...) {
    std::cerr << "FATAL ERROR: Unknown error during DUT VExampleRocketSystem "
                 "instantiation."
              << std::endl;
    return 1;
  }
  ...
  try { // Wrap simulation loop in try-catch
    while (sim_time_steps < max_sim_time_steps && !contextp->gotFinish()) {
      ...
      // --- Time Management ---
      // Simplified time management as there's no clock signal
      contextp->time(sim_time_steps); // Use logical steps as time
      global_time = contextp->time(); // Update global time

      // --- Stimulus Generation (User Placeholder) ---
      // TODO: Add stimulus generation logic here if needed (without
      // clock/reset) Example: Apply some input based on sim_time_steps if
      // (sim_time_steps > 5) { // Example condition
      //    // top->io_some_input = ...;
      // }
  ...
  if (!contextp
    ...
    contextp->time(sim_time_steps); // Set final time based on steps
