...
int main(int argc, char **argv) {
    ...
    // Instantiate DUT
    std::unique_ptr<VMulDiv> top;
    try {
        top = std::make_unique<VMulDiv>(contextp.get(), "DUT");
    } catch (const std::exception& e) {
         std::cerr << "FATAL ERROR: Failed to instantiate DUT VMulDiv: " << e.what() << std::endl;
         return 1;
    } catch (...) {
         std::cerr << "FATAL ERROR: Unknown error during DUT VMulDiv instantiation." << std::endl;
         return 1;
    }
