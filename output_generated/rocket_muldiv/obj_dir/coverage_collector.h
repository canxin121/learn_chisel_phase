...
private:
  ...
  VMulDiv *dut_ptr = nullptr;

public:
  inline CoverageCollector();
  inline void initialize(VMulDiv *top);
  inline void update();
...
inline void CoverageCollector::initialize(VMulDiv *top) {
  if (!top) {
    throw std::runtime_error(
        "Error: DUT pointer is null in CoverageCollector::initialize.");
  }
  dut_ptr = top;
  std::cout << "CoverageCollector: Initializing coverage points for VMulDiv..."
            << std::endl;
