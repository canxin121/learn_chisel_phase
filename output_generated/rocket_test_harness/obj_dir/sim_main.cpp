...
int main(int argc, char **argv) {
  ...
  // Instantiate DUT
  std::unique_ptr<VTestHarness> top;
  try {
    top = std::make_unique<VTestHarness>(contextp.get(), "DUT");
  } catch (const std::exception &e) {
    std::cerr << "FATAL ERROR: Failed to instantiate DUT VTestHarness: "
              << e.what() << std::endl;
    return 1;
  } catch (...) {
    std::cerr
        << "FATAL ERROR: Unknown error during DUT VTestHarness instantiation."
        << std::endl;
    return 1;
  }
