...
done_processing:
  ...
  try {
    coverage_collector.initialize(tile);
    std::cout << "CoverageCollector initialized." << std::endl;
  } catch (const std::exception &e) {
    std::cerr << "FATAL ERROR: Failed to initialize CoverageCollector: "
              << e.what() << std::endl;
#if VM_TRACE
    if (vcdfile && vcdfile != stdout) {
        fclose(vcdfile);
    }
#endif
    if (tile) delete tile;
  ...
  while (trace_count < max_cycles) {
    ...
    if (done_reset) {
        try {
            ...
            std::cerr << "ERROR during coverage_collector.update() @ cycle " << trace_count
                      << ": " << e.what() << std::endl;
            ret = 1; // Indicate an error occurred
            fprintf(stderr, "*** FAILED *** due to coverage collector error (seed %d) after %ld cycles\n", random_seed, trace_count);
  ...
  try {
    ...
    std::cout << "Coverage report exported to coverage_report.json" << std::endl;
  } catch (const std::exception &e) {
