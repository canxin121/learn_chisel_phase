...
int main(int argc, char **argv) {
  ...
  // Instantiate DUT
  std::unique_ptr<VRegModule> top;
  try {
    top = std::make_unique<VRegModule>(contextp.get(), "DUT");
  } catch (const std::exception &e) {
    std::cerr << "FATAL ERROR: Failed to instantiate DUT VRegModule: "
              << e.what() << std::endl;
    return 1;
  } catch (...) {
    std::cerr
        << "FATAL ERROR: Unknown error during DUT VRegModule instantiation."
        << std::endl;
    return 1;
  }
  ...
  try { // Wrap simulation loop in try-catch
    while (sim_time_steps < max_sim_time_steps && !contextp->gotFinish()) {
      ...
      if (sim_time_steps <
        ...
        top->reset = 1;
        top->io_sel = 0; // Initialize inputs during reset
      ...
      // Add stimulus generation logic here based on sim_time_steps or global_time
      if (top->reset == 0) {
        // Example: Toggle io_sel every 20 steps after reset release
        if ((sim_time_steps - 5) % 20 == 0) {
           top->io_sel = !top->io_sel; // Toggle the selection signal
           std::cout << "[" << global_time << "] Toggling io_sel to " << (int)top->io_sel << std::endl;
        }
      }
      ...
      // --- Coverage Update ---
      // Only update coverage when not in reset
      if (top->reset == 0) {
