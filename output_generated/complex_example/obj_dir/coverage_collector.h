...
private:
  ...
  VComplexExample *dut_ptr = nullptr;

public:
  inline CoverageCollector();
  inline void initialize(VComplexExample *top);
  inline void update();
...
inline void CoverageCollector::initialize(VComplexExample *top) {
  if (!top) {
    throw std::runtime_error(
        "Error: DUT pointer is null in CoverageCollector::initialize.");
  }
  dut_ptr = top;
  std::cout << "CoverageCollector: Initializing coverage points for VComplexExample..."
            << std::endl;
  ...
  // Register signal: _rs__s1 (UInt)
        register_points.emplace_back("_rs__s1", 1, reinterpret_cast<const void*>(&top->_rs___05Fs1));
