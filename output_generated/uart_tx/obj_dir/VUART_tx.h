...
#ifndef VERILATED_VUART_TX_H_
#define VERILATED_VUART_TX_H_  // guard

#include "verilated.h"

class VUART_tx__Syms;
class VUART_tx___024root;
class VerilatedVcdC;

// This class is the main interface to the Verilated model
class alignas(VL_CACHE_LINE_BYTES) VUART_tx VL_NOT_FINAL : public VerilatedModel {
  private:
  ...
  public:
    ...
    VUART_tx___024root* const rootp;
    ...
    explicit VUART_tx(VerilatedContext* contextp, const char* name = "TOP");
    explicit VUART_tx(const char* name = "TOP");
    /// Destroy the model; called (often implicitly) by application code
    virtual ~VUART_tx();
  private:
    VL_UNCOPYABLE(VUART_tx);  ///< Copying not allowed
