...
int main(int argc, char **argv) {
    ...
    // Instantiate DUT
    std::unique_ptr<V{{TOP_MODULE_NAME}}> top;
    try {
        top = std::make_unique<V{{TOP_MODULE_NAME}}>(contextp.get(), "DUT");
    } catch (const std::exception& e) {
         std::cerr << "FATAL ERROR: Failed to instantiate DUT V{{TOP_MODULE_NAME}}: " << e.what() << std::endl;
         return 1;
    } catch (...) {
         std::cerr << "FATAL ERROR: Unknown error during DUT V{{TOP_MODULE_NAME}} instantiation." << std::endl;
         return 1;
    }
